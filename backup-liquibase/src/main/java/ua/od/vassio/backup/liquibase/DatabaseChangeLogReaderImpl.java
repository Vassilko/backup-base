...
public class DatabaseChangeLogReaderImpl implements DatabaseChangeLogReader {
    ...
    public void read(DatabaseChangeLog databaseChangeLog, String path, Comparator<String> comporator) throws IOException, LiquibaseException {
        ...
        Collection<String> unsortedResources = resourceAccessor.list(path, path, true, false, false);

        if (CollectionUtils.isNotEmpty(unsortedResources)) {
            Collection<String> resources = new TreeSet<String>(comporator);
            resources.addAll(unsortedResources);

            for (String file : resources) {
                DatabaseChangeLog changeLog = ChangeLogParserFactory.getInstance().getParser(file, resourceAccessor).parse(file, changeLogParameters, resourceAccessor);
